...
This module is built on Express, while this API is Fastify (§11). `@fastify/express`
bridges it into the same Fastify application and the same deployment — no second process,
no second origin. **As built (slice 1), the bridge carries the SDK's lower-level
`authorizationHandler`/`tokenHandler` request logic directly, not the bundled
`mcpAuthRouter`**: that router assumes mounting at the application's root and hardcodes
`/register`/`/revoke` alongside `/authorize`/`/token`, which is incompatible with mounting
under this deployment's own `/api/v1` prefix ahead of Dynamic Client Registration existing
(slice 3) — using it as originally described here would have meant standing up routes this
slice does not yet support. `src/oauth/routes.ts` documents the mounting constraint this
forces in detail: the bridge must be installed on the *top-level* Fastify app with the full
absolute path baked in, never inside the `{ prefix }`-registered block the rest of this
API's routes share, or requests 404 silently. The resource server's own bearer-token check
(verifying a request to the MCP endpoint itself, §7.9) does **not** need this bridge: it is
a single async call to `verifyAccessToken`, wired as an ordinary Fastify `preHandler`,
exactly like `bearerAuthRoute` already wires JWT checks today (§4.1).
...
**As built (slice 1), both documents are hand-assembled, not SDK-generated** — the SDK's
own metadata builder hardcodes its endpoints as root-relative paths (`/authorize`,
`/token`), which the WHATWG URL spec's own resolution rules mean no `baseUrl` can place
under this deployment's `/api/v1/oauth/*` prefix, so it cannot produce a correct document
for an API mounted under a path prefix at all. Only the ~10-field static document is
hand-maintained; the endpoints it names are still served by the SDK's own
`authorizationHandler`/`tokenHandler` request logic (§4.3.1), so the behavior those
documents describe is not hand-rolled, only their assembly into JSON is. `registration_endpoint`
is correctly omitted from the RFC 8414 document until slice 3 builds Dynamic Client
Registration — naming an endpoint that does not exist would be actively wrong.
...
- **The OAuth 2.1 authorization server core for third-party and MCP clients, slice 1 of
  §4.3/§7.9's build order** (`513ee16`, with design-review fixes in `7d127c7`, `cb6f34c`,
  `a0a9a68`): `GET /oauth/authorize`, `POST /oauth/token`, PKCE (`S256` only), RFC 8707
  resource indicators, and Client ID Metadata Document client resolution — including its
  SSRF guard (a `node:net` `BlockList`-based address classifier, table-driven per blocked
  CIDR) and connect-time DNS-rebinding protection (an `undici` dispatcher whose `lookup`
  re-validates the address actually connected to, not merely the one first resolved). Both
  discovery documents are implemented at their RFC-correct locations: RFC 8414's Authorization
  Server Metadata at the bare `/.well-known/oauth-authorization-server` (§4.3.5 — this AS's
  `issuer` has no path component, so no suffix applies) and RFC 9728's Protected Resource
  Metadata at the path-suffixed `/.well-known/oauth-protected-resource/api/v1/mcp` (§4.3.5 —
  the bare path 404s, as specified). §4.3.6's audience MUST is enforced, not merely stated:
  `authenticatedVoterId` refuses any token carrying `aud`, so an AS-minted, resource-bound
  token authenticates nothing on the existing REST surface. §4.3.7's refresh-token audience
  binding is implemented in both directions: `refresh_tokens.resource` (nullable) is set once
  at issuance and never changed; `POST /auth/refresh` refuses a bound family (`401`, family
  revoked); `POST /oauth/token`'s `refresh_token` grant compares the stored `resource` against
  the requested one exactly. Rate limiting on `/oauth/authorize` and `/oauth/token` uses the
  SDK's own `express-rate-limit` defaults rather than being switched off; its per-client
  accuracy behind App Platform's ingress is a recorded, open limitation
  (`war-infra-spec.md` §5.2) rather than a solved problem. Slice 1's own
  build order item (§4.3's "Build order" list) is complete.

**Not built, and not to be inferred from what's built above:**
- `oauth_clients` (§6) does not exist as a table — Client ID Metadata Documents (slice 1,
  above) need no stored client row, and that table exists only for Dynamic Client
  Registration, which is slice 3. Only `authorization_codes` and `refresh_tokens.resource`
  are real, migrated schema today.
- `POST /oauth/register` (Dynamic Client Registration, RFC 7591) — slice 3.
- **The MCP interface itself does not exist.** `/api/v1/mcp`, the nine-tool (soon ten,
  §7.9) surface, and the service-layer allowlist module (`src/mcp/allowedActions.ts`) and
  its two enforcement checks are all slice 2, unbuilt. A client can complete the OAuth
  dance slice 1 provides and receive a resource-bound access token; there is nothing at the
  resource it names for that token to be used against yet.
- Issuer validation hardening, `WWW-Authenticate` scope challenges, and revocation wiring —
  slice 4.

The existing browser flow (§4.1, §4.2) is unaffected by any of the above, built or not.
**Two designs previously occupied this slot in this document and were discarded outright,
not merged**: a Playwright-driven cookie-extraction design, and a loopback/native-app
(RFC 8252) design for a locally-run MCP process — both assumed a local process, which a
deployed MCP server accessible from Claude's desktop and mobile apps is not. Neither left a
trace here beyond this note and the revision notes at §4.3 and §7.9.

**Not yet implemented (unrelated to §4.3/§7.9):**
...
- Rate limiting (§9.4) — the edge's volumetric limits (`war-infra-spec.md` §13.1) are live;
  the API's own per-voter, per-vote limits described here are not (distinct from §4.3's own
  OAuth-endpoint rate limiting, above, which is built)
...
1. **Complete (`513ee16`, `7d127c7`, `cb6f34c`, `a0a9a68`). The authorization server core,
   with CIMD as its registration mechanism** — `/oauth/authorize`, `/oauth/token`, PKCE,
   resource indicators (RFC 8707), Client ID Metadata Document resolution (§4.3.3,
   including its SSRF guards), and the two discovery documents. Provable against a real
   `https://` CIMD document without the MCP endpoint existing yet, and — unlike a
   DCR-first sequencing — provable against whatever registration mechanism a real MCP
   client actually speaks, since CIMD needs no prior registration call at all.
