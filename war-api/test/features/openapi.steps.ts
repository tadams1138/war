 ...
 * shapes), and the document's own endpoint (not self-described). Built from
 * Fastify's own routing table, not a hand-copied list, so this is a real
 * drift check rather than a restatement of the spec. Compares at method
 * granularity, not path alone, so a route silently losing a verb (e.g.
 * `POST /wars` dropping out while `GET /wars` remains) still fails.
 */
